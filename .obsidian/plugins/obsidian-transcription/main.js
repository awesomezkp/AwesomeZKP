  ...
  async getTranscriptionWhisperASR(file) {
    ...
    return (0, import_obsidian2.requestUrl)(options).then(async (response) => {
      ...
      if (this.settings.timestamps)
        return this.segmentsToTimestampedString(response.json.segments, this.settings.timestampFormat);
    ...
    while (true) {
      ...
      if (transcription.status == "transcribed") {
        ...
        if (!transcription.text_segments || !transcription.text) {
          if (this.settings.debug)
            console.error("Scribe returned an invalid transcription");
          return Promise.reject("Scribe returned an invalid transcription");
        }
        if (this.settings.timestamps)
          return this.segmentsToTimestampedString(transcription.text_segments, this.settings.timestampFormat);
        else
...
var TranscriptionSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("Timestamp format").setDesc("The format of the timestamps: date-fns.org/docs/format").addDropdown((dropdown) => dropdown.addOption("HH:mm:ss", "HH:mm:ss").addOption("mm:ss", "mm:ss").addOption("ss", "ss").setValue(this.plugin.settings.timestampFormat).onChange(async (value) => {
